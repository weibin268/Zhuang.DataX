...
namespace Zhuang.Data.Entity.Mapping
{
    public class TableMapping
    {
        ...
        private void Init()
        {
            ...
            var tableAttributes = _typeEntity.GetTypeInfo().GetCustomAttributes(typeof(TableAttribute), false).ToArray();
            ...
            foreach (var pi in _typeEntity.GetTypeInfo().GetProperties())
            {

                var ignoreAttributes = pi.GetCustomAttributes(typeof(IgnoreAttribute), false).ToArray();
                ...
                var columnAttributes = pi.GetCustomAttributes(typeof(ColumnAttribute), false).ToArray();
                ...
                var keyAttributes = pi.GetCustomAttributes(typeof(KeyAttribute), false).ToArray();
                if (keyAttributes.Length > 0)
                {
                    column.IsPrimaryKey = true;
                }

                var autoAttributes = pi.GetCustomAttributes(typeof(AutoGenerateAttribute), false).ToArray();
        ...
        public void FilterColumn(object objEntity)
        {
            ...
            {
                foreach (var pi in objEntity.GetType().GetTypeInfo().GetProperties())
                {
